...
| batch-btns               | [1.24.0 新增] 通用 - 底部批量操作按钮列表 [查看 batch-btns](#batch-btns)                                                                                    | Array               | []                                                 | -                                    |
| pagination-placement     | [1.24.0 新增] 通用 - 分页器位置（当 batchBtns 有值时固定为 bottomEnd）                                                                                      | String              | bottomEnd                                          | bottomStart、bottomCenter、bottomEnd |
...
底部批量操作按钮，显示在分页器同行左侧，需配合 `selection` 使用。支持两种模式：
