...
- 5、[连表查询，主表外键是数组（id数组），查询出数组内的每个记录详情](#scene5)
- 6、[连表查询，副表外键是数组（只要数组内任意元素与主表外键匹配即可）](#scene6)
- 7、[分组查询](#scene7)
- 8、[分组统计带if(查询每个班级中，数学成绩大于语文成绩的学生人数)](#scene8)
...
### vk.baseDao.getTableData的连表@getTableData
...
### 场景2：1张主表多张副表，同时副表也有多张副表@scene2
...
### 场景3：需要查询同时满足主表和副表的条件，即副表不满足条件，则主表记录也不获取@scene3
...
### 场景4：连表查询时需要按照离给定点从近到远输出（地理位置经纬度）@scene4
...
### 场景5：连表查询，主表外键是数组（id数组），查询出数组内的每个记录详情@scene5
...
### 场景6：连表查询，副表外键是数组（只要数组内任意元素与主表外键匹配即可）@scene6
...
### 场景9：连表查询，并获取满足条件的第一条记录，以对象形式返回@scene9
...
### 场景10：利用分组查询实现以指定字段去重复查询@scene10
