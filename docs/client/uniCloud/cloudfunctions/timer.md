...
自 vk-unicloud 2.21.0 版本起，router 函数支持定时任务方式运行，具体操作如下

1. 从最新版示例项目中复制目录 `router/service/crontab/` 到你的项目中（2.21.0版本起已内置此目录）
2. 修改 `router/package.json` 文件，在 `cloudfunction-config` 配置项中新增 `triggers` 配置，具体如下
```js
...
timeout 超时时间，腾讯最大900秒，阿里云7200秒，支付宝云10800秒

triggers 定时触发器配置，注意：只支持配置1个，即数组内只能有一个元素

config 触发器Cron 表达式，[详见](#cron)
