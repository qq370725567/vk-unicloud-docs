...
### 初始化 Agent 配置@init-agent

用于生成 AI 编程工具需要的项目环境配置，适用于插件 0.13.0 及以上版本。

**使用方式**

1. 将 [VK 框架文档仓库](https://gitee.com/vk-uni/vk-unicloud-docs.git) 导入到与当前项目相同的父目录下，保留目录名 `vk-unicloud-docs`。
2. 在 HBuilderX 项目管理器中右键 VK 框架项目名，选择【VK】-【初始化 Agent 配置】。也可以在项目根目录的 `AGENTS.md` 文件或该文件的代码编辑器中右键执行。
3. 初始化完成后，点击【查看】检查生成或更新的 `AGENTS.md`。

工具会自动识别 uniCloud 目录、router 主函数名和文档根目录，写入项目根目录的 `AGENTS.md`，并创建 `CLAUDE.md` 或补充 `@AGENTS.md` 引用。支持重复执行以更新环境信息，并保留已有的其他规则。

> 菜单仅在项目包含 `uni_modules/vk-unicloud/` 时显示。项目根目录的 `app.config.js` 需配置有效的 `functionName`。旧版【初始化环境变量】已由此功能替代；旧文件的处理方式、配置示例及不同 AI 工具的使用方法见 [AI 编程 - 初始化 Agent 配置](./aiCoding.md#init-env)。

### 初始化项目配置文件@初始化项目文件

用于在项目根目录快速补充常用的 Git、Prettier、HBuilderX 格式化及 npm 发布配置文件。插件 0.13.0 起，原【初始化项目文件】菜单更名为【初始化项目配置文件】。

**使用方式**

1. 在 HBuilderX 项目管理器中右键项目根目录，选择【VK】-【初始化项目配置文件】。
